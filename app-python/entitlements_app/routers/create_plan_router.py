"""FastAPI router for command `CreatePlan`."""
...
_AUTHORIZATION_OPERATION = "CreatePlan"
...
def handle(request: CreatePlanRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["entitlement-admin"], ["entitlements:access", "entitlements:write"]))):
    service = CreatePlanService(entitlements_repository=entitlements_repository)
