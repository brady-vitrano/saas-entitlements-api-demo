"""FastAPI router for query `GetFeature`."""
...
_AUTHORIZATION_OPERATION = "GetFeature"
...
@router.get("/api/features/{featureKey}")
def handle(featureKey: str, tenantId: str, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access([], ["entitlements:access"]))):
    request = GetFeatureQueryRequest(featureKey=featureKey, tenantId=tenantId)
    service = GetFeatureQueryService(entitlements_repository=entitlements_repository)
