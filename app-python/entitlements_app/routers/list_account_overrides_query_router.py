"""FastAPI router for query `ListAccountOverrides`."""
...
_AUTHORIZATION_OPERATION = "ListAccountOverrides"
...
@router.get("/api/accounts/{accountId}/overrides")
def handle(accountId: str, tenantId: str, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access([], ["entitlements:access"]))):
    request = ListAccountOverridesQueryRequest(accountId=accountId, tenantId=tenantId)
    service = ListAccountOverridesQueryService(entitlements_repository=entitlements_repository)
