"""FastAPI router for command `RegisterFeature`."""
...
_AUTHORIZATION_OPERATION = "RegisterFeature"
...
@router.post("/api/features", status_code=201)
def handle(request: RegisterFeatureRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["entitlement-admin"], ["entitlements:access", "entitlements:write"]))):
    service = RegisterFeatureService(entitlements_repository=entitlements_repository)
