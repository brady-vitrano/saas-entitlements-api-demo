"""FastAPI router for command `AppendAuditEvent`."""
...
_AUTHORIZATION_OPERATION = "AppendAuditEvent"
...
@router.post("/api/audit-events", status_code=201)
def handle(request: AppendAuditEventRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["account-operator"], ["entitlements:access", "entitlements:write"]))):
    service = AppendAuditEventService(entitlements_repository=entitlements_repository)
    ...
    if str(request.tenantId) != _authorization_subject(principal, "authenticated_tenant_id"):
    ...
    if not _claim_matches(principal, "allowed_account_ids", request.accountId, "contains_input"):
