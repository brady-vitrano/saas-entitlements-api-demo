"""FastAPI router for command `AddFeatureToPlan`."""


from fastapi import APIRouter, Depends, HTTPException, status

from entitlements_app.requests.add_feature_to_plan_request import AddFeatureToPlanRequest
from entitlements_app.requests.add_feature_to_plan_http_request import AddFeatureToPlanHttpRequest
...
_AUTHORIZATION_POLICY_ID = "PlanAdminPolicy"
_AUTHORIZATION_RULE_ID = "plan-admin-write"
_AUTHORIZATION_POLICY_IDS = ["PlanAdminPolicy", "TenantBoundaryPolicy"]
_AUTHORIZATION_RULE_IDS = ["plan-admin-write", "tenant-boundary"]
_AUTHORIZATION_TARGET = "commands"
_AUTHORIZATION_OPERATION = "AddFeatureToPlan"
...
@router.post("/api/plans/{planId}/features", status_code=201)
def handle(planId: str, request: AddFeatureToPlanHttpRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["entitlement-admin"], ["entitlements:access", "entitlements:write"]))):
    service = AddFeatureToPlanService(entitlements_repository=entitlements_repository)
    service_request = AddFeatureToPlanRequest(allowance=request.allowance, effectiveAt=request.effectiveAt, enforcementMode=request.enforcementMode, entitlementId=request.entitlementId, featureKey=request.featureKey, planId=planId, tenantId=principal.tenant_id or principal.subject, unit=request.unit)
