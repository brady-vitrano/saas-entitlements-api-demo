"""FastAPI router for query `ListAccountAuditEvents`."""
...
_AUTHORIZATION_OPERATION = "ListAccountAuditEvents"
...
@router.get("/api/accounts/{accountId}/audit-events")
def handle(accountId: str, tenantId: str, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access([], ["entitlements:access"]))):
    request = ListAccountAuditEventsQueryRequest(accountId=accountId, tenantId=tenantId)
    service = ListAccountAuditEventsQueryService(entitlements_repository=entitlements_repository)
