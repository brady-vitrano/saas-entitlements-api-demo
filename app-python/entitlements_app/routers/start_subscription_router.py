"""FastAPI router for command `StartSubscription`."""
...
_AUTHORIZATION_OPERATION = "StartSubscription"
...
def handle(request: StartSubscriptionRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["account-operator"], ["entitlements:access", "entitlements:write"]))):
    service = StartSubscriptionService(entitlements_repository=entitlements_repository)
