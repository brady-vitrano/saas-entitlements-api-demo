"""FastAPI router for command `RecordUsage`."""
...
_AUTHORIZATION_OPERATION = "RecordUsage"
...
@router.post("/api/usage", status_code=201)
def handle(request: RecordUsageRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["account-operator"], ["entitlements:access", "entitlements:write"]))):
    service = RecordUsageService(entitlements_repository=entitlements_repository)
