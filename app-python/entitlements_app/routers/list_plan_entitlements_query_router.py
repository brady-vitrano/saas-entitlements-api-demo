"""FastAPI router for query `ListPlanEntitlements`."""
...
_AUTHORIZATION_OPERATION = "ListPlanEntitlements"
...
@router.get("/api/plans/{planId}/entitlements")
def handle(planId: str, tenantId: str, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access([], ["entitlements:access"]))):
    request = ListPlanEntitlementsQueryRequest(planId=planId, tenantId=tenantId)
    service = ListPlanEntitlementsQueryService(entitlements_repository=entitlements_repository)
