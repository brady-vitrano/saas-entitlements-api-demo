"""FastAPI router for command `CheckFeatureAccess`."""
...
_AUTHORIZATION_OPERATION = "CheckFeatureAccess"
...
@router.post("/api/entitlements/check")
def handle(request: CheckFeatureAccessRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["account-operator"], ["entitlements:access", "entitlements:write"]))):
    service = CheckFeatureAccessService(entitlements_repository=entitlements_repository)
