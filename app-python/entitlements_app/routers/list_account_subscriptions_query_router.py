"""FastAPI router for query `ListAccountSubscriptions`."""
...
from entitlements_app.services.list_account_subscriptions_query_service import ListAccountSubscriptionsQueryService
...
_AUTHORIZATION_OPERATION = "ListAccountSubscriptions"
...
def handle(accountId: str, tenantId: str, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access([], ["entitlements:access"]))):
    request = ListAccountSubscriptionsQueryRequest(accountId=accountId, tenantId=tenantId)
    service = ListAccountSubscriptionsQueryService(entitlements_repository=entitlements_repository)
