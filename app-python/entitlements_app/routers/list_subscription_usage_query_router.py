"""FastAPI router for query `ListSubscriptionUsage`."""
...
_AUTHORIZATION_OPERATION = "ListSubscriptionUsage"
...
@router.get("/api/subscriptions/{subscriptionId}/usage")
def handle(subscriptionId: str, tenantId: str, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access([], ["entitlements:access"]))):
    request = ListSubscriptionUsageQueryRequest(subscriptionId=subscriptionId, tenantId=tenantId)
    service = ListSubscriptionUsageQueryService(entitlements_repository=entitlements_repository)
