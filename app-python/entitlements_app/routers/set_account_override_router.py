"""FastAPI router for command `SetAccountOverride`."""


from fastapi import APIRouter, Depends, HTTPException, status

from entitlements_app.requests.set_account_override_request import SetAccountOverrideRequest
from entitlements_app.requests.set_account_override_http_request import SetAccountOverrideHttpRequest
...
_AUTHORIZATION_POLICY_ID = "AccountOperatorPolicy"
_AUTHORIZATION_RULE_ID = "account-subscription-write"
_AUTHORIZATION_POLICY_IDS = ["AccountOperatorPolicy", "TenantBoundaryPolicy"]
_AUTHORIZATION_RULE_IDS = ["account-subscription-write", "tenant-boundary"]
_AUTHORIZATION_TARGET = "commands"
_AUTHORIZATION_OPERATION = "SetAccountOverride"
...
@router.post("/api/accounts/{accountId}/overrides", status_code=201)
def handle(accountId: str, request: SetAccountOverrideHttpRequest, entitlements_repository: EntitlementsRepository = Depends(get_entitlements_repository), principal: AuthenticatedPrincipal = Depends(require_access(["account-operator"], ["entitlements:access", "entitlements:write"]))):
    service = SetAccountOverrideService(entitlements_repository=entitlements_repository)
    service_request = SetAccountOverrideRequest(accountId=accountId, allowance=request.allowance, createdAt=request.createdAt, expiresAt=request.expiresAt, featureKey=request.featureKey, overrideId=request.overrideId, overrideType=request.overrideType, reason=request.reason, tenantId=principal.tenant_id or principal.subject)
    ...
    if "entitlements:write" not in principal.scopes:
        _authorization_failure("scopeCheck", "scope entitlements:write required")
    # authorization: authorizationGuard.tenantMatch input=tenantId source=authenticated_tenant_id check=match_input
    if str(service_request.tenantId) != _authorization_subject(principal, "authenticated_tenant_id"):
        _authorization_failure("tenantMatch", "tenant boundary required")
    # authorization: authorizationGuard.claimMatch claim=allowed_account_ids input=accountId check=contains_input
    if not _claim_matches(principal, "allowed_account_ids", service_request.accountId, "contains_input"):
        _authorization_failure("claimMatch", "claim constraint required")
    return service.handle(service_request)