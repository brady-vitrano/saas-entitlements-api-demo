...
> **Disclaimer:** The views and opinions expressed in this repository are solely my own and do not represent the views or opinions of Amazon or its affiliates.
...
The apps are generated from the same private spec set and published here as a standalone public demo. Raw input specs are intentionally not included because their schema headers contain private generation identifiers; this repo contains the generated app code, neutral metrics, Docker proof evidence, endpoint receipts, and Spring-vs-FastAPI happy-path comparison receipts.
...
- **Happy-path equivalence verified, not full wire parity**: both Docker-started apps ran the same 10-call API flow; `10 / 10` statuses matched and `10 / 10` canonical JSON payloads matched. Full over-the-wire parity is not claimed because the public HTTP contract surfaces still differ.
...
Timing details are in `generation-metrics.json`. The broader task timer starts at the earliest reliable private spec artifact timestamp, `2026-06-03T03:35:43Z`, and ends during comparison README/metrics handoff at `2026-06-03T14:51:07Z`; it is marked approximate for that reason.
...
The comparison proof covers HTTP status codes and canonicalized JSON response bodies for ten happy-path calls. Result: `10 / 10` status matches and `10 / 10` payload matches.

This is not a full over-the-wire parity proof. Known contract gaps:

- FastAPI exposes `/openapi.json`; the Spring app returns 404 for `/v3/api-docs` in this demo build.
- The proof does not compare every response header, validation error body, auth failure shape, OpenAPI schema, or unsupported endpoint behavior.
- FastAPI generation required `--skip-doctor` because preflight reported role/scope guard capabilities as unsupported, even though the Docker runtime flow passed.

Comparison evidence:

- `evidence/comparison/report.txt`
- `evidence/comparison/java/`
- `evidence/comparison/python/`
...
The final endpoint comparison returned `0`; all ten API calls returned matching 2xx statuses and matching canonical JSON payloads. See:
...
- `evidence/comparison/` (cross-stack happy-path comparison)
...
The FastAPI preflight reported unsupported role/scope guard capabilities. The submitted run used the documented `--skip-doctor` flag, and the Docker runtime plus happy-path payload comparison passed.
