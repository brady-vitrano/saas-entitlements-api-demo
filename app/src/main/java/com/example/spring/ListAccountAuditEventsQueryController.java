...
/**
 * REST surface for query ListAccountAuditEvents.
 */
@RestController
public class ListAccountAuditEventsQueryController {

    private final ListAccountAuditEventsQueryService service;

    @Autowired
    public ListAccountAuditEventsQueryController(ListAccountAuditEventsQueryService service) {
        this.service = service;
    }

    @GetMapping("/api/accounts/{accountId}/audit-events")
    public java.util.List<AuditEvent> handle(
        ...
        if (!authorizationValueEquals(tenantId, authorizationSubject(authentication, "authenticated_tenant_id"))) {
            handleAuthorizationFailure("tenantMatch", "tenant boundary required");
        }
        return service.handle(new ListAccountAuditEventsQueryRequest(accountId, tenantId));
    }
    ...
    private String authorizationDecisionReceipt(
        ...
        return "{"
                ...
                + "\"policyId\":\"TenantBoundaryPolicy\","
                + "\"ruleId\":\"tenant-boundary\","
                + "\"policyIds\":[\"TenantBoundaryPolicy\"],"
                + "\"ruleIds\":[\"tenant-boundary\"],"
                + "\"target\":\"queries\","
