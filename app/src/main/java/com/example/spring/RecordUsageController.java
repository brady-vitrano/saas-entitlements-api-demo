...
/**
 * REST surface for command RecordUsage.
 */
@RestController
public class RecordUsageController {

    private final RecordUsageService service;


    @Autowired
    public RecordUsageController(RecordUsageService service) {
        this.service = service;
    }

    @PreAuthorize("hasRole('account-operator')")
    @PostMapping("/api/usage")
    public ResponseEntity<UsageMeter> handle(@Valid @RequestBody RecordUsageRequest request, Authentication authentication) {
