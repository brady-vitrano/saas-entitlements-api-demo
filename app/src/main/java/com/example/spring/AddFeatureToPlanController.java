...
/**
 * REST surface for command AddFeatureToPlan.
 */
@RestController
public class AddFeatureToPlanController {

    private final AddFeatureToPlanService service;


    @Autowired
    public AddFeatureToPlanController(AddFeatureToPlanService service) {
        this.service = service;
    }

    @PreAuthorize("hasRole('entitlement-admin')")
    @PostMapping("/api/plans/{planId}/features")
    public ResponseEntity<Entitlement> handle(@PathVariable String planId, @Valid @RequestBody AddFeatureToPlanHttpRequest body, Authentication authentication) {
        AddFeatureToPlanRequest request = new AddFeatureToPlanRequest(body.allowance(), body.effectiveAt(), body.enforcementMode(), body.entitlementId(), body.featureKey(), planId, body.tenantId(), body.unit());
