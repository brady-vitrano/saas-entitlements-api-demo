...
/**
 * REST surface for command SetAccountOverride.
 */
@RestController
public class SetAccountOverrideController {

    private final SetAccountOverrideService service;


    @Autowired
    public SetAccountOverrideController(SetAccountOverrideService service) {
        this.service = service;
    }

    @PreAuthorize("hasRole('account-operator')")
    @PostMapping("/api/accounts/{accountId}/overrides")
    public ResponseEntity<AccountOverride> handle(@PathVariable String accountId, @Valid @RequestBody SetAccountOverrideHttpRequest body, Authentication authentication) {
        SetAccountOverrideRequest request = new SetAccountOverrideRequest(accountId, body.allowance(), body.createdAt(), body.expiresAt(), body.featureKey(), body.overrideId(), body.overrideType(), body.reason(), body.tenantId());
        ...
        if (!identityClaimMatches(authentication, "allowed_account_ids", request.accountId(), "contains_input")) {
            handleAuthorizationFailure("claimMatch", "claim constraint required");
        }
    ...
    private String authorizationDecisionReceipt(
        ...
        return "{"
                ...
                + "\"policyId\":\"AccountOperatorPolicy\","
                + "\"ruleId\":\"account-subscription-write\","
                + "\"policyIds\":[\"AccountOperatorPolicy\",\"TenantBoundaryPolicy\"],"
                + "\"ruleIds\":[\"account-subscription-write\",\"tenant-boundary\"],"
                + "\"target\":\"commands\","
                + "\"operation\":\"SetAccountOverride\","
