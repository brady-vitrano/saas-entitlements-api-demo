...
/**
 * REST surface for command AppendAuditEvent.
 */
@RestController
public class AppendAuditEventController {

    private final AppendAuditEventService service;


    @Autowired
    public AppendAuditEventController(AppendAuditEventService service) {
        this.service = service;
    }

    @PreAuthorize("hasRole('account-operator')")
    @PostMapping("/api/audit-events")
    public ResponseEntity<AuditEvent> handle(@Valid @RequestBody AppendAuditEventRequest request, Authentication authentication) {
