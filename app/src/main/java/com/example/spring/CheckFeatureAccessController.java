...
/**
 * REST surface for command CheckFeatureAccess.
 */
@RestController
public class CheckFeatureAccessController {

    private final CheckFeatureAccessService service;


    @Autowired
    public CheckFeatureAccessController(CheckFeatureAccessService service) {
        this.service = service;
    }

    @PreAuthorize("hasRole('account-operator')")
    @PostMapping("/api/entitlements/check")
    public PolicyCheckResult handle(@Valid @RequestBody CheckFeatureAccessRequest request, Authentication authentication) {
